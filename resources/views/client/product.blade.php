...
<div class="fruit-productt">
    <h1 style="text-align:center;margin-bottom:70px;font-size:40px;">{{$title}}</h1>
    <div >
    </div>
    <div class="fruit-productt__list">
        ...
        <div class="fruit-productt__item">
            <a href="{{url('/chi_tiet_san_pham/'.$item->mahh)}}"><img src="{{asset($item->anh)}}" alt="" class="fruit-product__image"></a>
            <h3 class="fruit-productt__name">{{$item->tenhh}}</h3>
            <p class="fruit-productt__price">{{ number_format($item->dongia, 0) }}<sup>đ</sup></p>
            @if(Auth::check())
            <button class="fruit-productt__btn"><a href="{{url('add-to-cart/'.$item->mahh)}}">THÊM VÀO GIỎ HÀNG</a></button>
            @else
